...
class PokemonDetails extends React.Component {
  ...
  render () {
    const {name, weight, height, abilities, sprites, moves, types} = this.props.details.pokemonDetails
    ...
    return (
      <React.Fragment>
        ...
        <div className='content'>
          <h4>Abilities</h4>
          <p>
            {arrayToList(abilities, 'ability.name')}
          </p>
          <h4>Moves</h4>
          <p>
            {arrayToList(moves, 'move.name')}
          </p>
        </div>
        <div className="text-center">
                <Link className="btn btn-primary" to="/master">&larr; Go back</Link>
        </div>
      </React.Fragment>
    )
  }
